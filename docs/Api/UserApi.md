...
| [**changeTeam()**](UserApi.md#changeTeam) | **PUT** /v1-beta/users/changeTeam | Change users to other team. |
...
```php
changeTeam($user_id, $change_team_request)
```
...
try {
    $apiInstance->changeTeam($user_id, $change_team_request);
} catch (Exception $e) {
